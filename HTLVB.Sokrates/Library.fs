...
type private SokratesWebService = XmlProvider<Schema = "sokrates.xsd">
...
%s{xmlParameters}
    ...
    let createHttpClient () =
        ...
        let cert = new X509Certificate2(config.ClientCertificate)
    ...
    let parseGender text =
        if String.eq text "m" then Male
        elif String.eq text "w" then Female
    ...
    member _.FetchClasses (schoolYear: int) = async {
        let! xmlElement = getRequestContent "getTSNClasses" [ "schoolYear", Simple (string schoolYear) ] |> fetch
        return parseClasses xmlElement
    }

    member this.FetchClasses () = async {
        return! this.FetchClasses(getSchoolYear DateTime.Today)
    }

    member _.FetchStudents (?date: DateTime) = async {
        let date = defaultArg date DateTime.Today
        ...
        return parseStudents mainStudentXmlData additionalStudentXmlData
    }

    member this.FetchStudents (className: string, ?date: DateTime) = async {
        let! students = this.FetchStudents(?date = date)
        return
            students
            |> List.filter (fun student -> String.eq student.SchoolClass className)
    }

    member _.FetchStudentAddresses (?date: DateTime) = async {
        let date = defaultArg date DateTime.Today
    ...
    member _.FetchStudentContactInfos (studentIds: SokratesId list, ?date: DateTime) = async {
